...
package org.sourceforge.EPSGraphics2D;
...
public class EpsGraphics2D extends java.awt.Graphics2D
{
	...
	public void setColor(Color c)
	{
		if(c == null){
			...
			System.out.println(getColorDepth());
		}
		...
		if(getColorDepth() == BLACK_AND_WHITE)
		{
			...
			{
				float value = ((c.getRed() + c.getGreen() + c.getBlue()) / (3 * 255.0f));
				append(value + " setgray");
			}
			else
				//System.out.println(c.getRed() + " " + c.getGreen() + " " + c.getBlue() + " setrgbcolor");
				append((c.getRed() / 255.0f) + " " + (c.getGreen() / 255.0f) + " " + (c.getBlue() / 255.0f) + " setrgbcolor");
	}
